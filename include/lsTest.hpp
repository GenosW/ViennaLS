...
struct lsTest {
  ...
  lsTest(const lsTestStatus stat) { status = stat; }
  lsTest(lsTestStatus stat, std::string nam) : status(stat), name(nam) {}

  template <typename T> bool run(T (&test)(void)) {
  ...
  lsTestStatus operator()() const { return this->status; }

  bool wasSuccess() const { return this->status == lsTestStatus::SUCCESS; }
};
...
template <typename T, typename... Args> bool all_equal(T target, Args... args) {
  return ((target == args) && ...);
}

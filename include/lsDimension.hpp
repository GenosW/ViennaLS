...
template <int D = 3> struct Dimension {
  using enum_type = uint;
  enum options : enum_type { START = 0, x = 1, y = 2, z, END = D + 1 };
  ...
  template <typename T = uint> T operator()() const {
  ...
  template <typename T = size_t> T toArrayIndex() const {
